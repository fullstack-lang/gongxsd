...
func (controller *Controller) GetA_SPEC_OBJECT_TYPE_REFs(c *gin.Context) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("GetA_SPEC_OBJECT_TYPE_REFs", "GONG__StackPath", stackPath)
		}
	}
	...
	query := db.Find(&a_spec_object_type_refDBs)
	if query.Error != nil {
...
func (controller *Controller) PostA_SPEC_OBJECT_TYPE_REF(c *gin.Context) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("PostA_SPEC_OBJECT_TYPE_REFs", "GONG__StackPath", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	query := db.Create(&a_spec_object_type_refDB)
	if query.Error != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = query.Error.Error()
...
func (controller *Controller) GetA_SPEC_OBJECT_TYPE_REF(c *gin.Context) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("GetA_SPEC_OBJECT_TYPE_REF", "GONG__StackPath", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if err := db.First(&a_spec_object_type_refDB, c.Param("id")).Error; err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
		log.Println(err.Error())
...
func (controller *Controller) UpdateA_SPEC_OBJECT_TYPE_REF(c *gin.Context) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("UpdateA_SPEC_OBJECT_TYPE_REF", "GONG__StackPath", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	query := db.First(&a_spec_object_type_refDB, c.Param("id"))
	...
	query = db.Model(&a_spec_object_type_refDB).Updates(a_spec_object_type_refDB)
	if query.Error != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = query.Error.Error()
...
func (controller *Controller) DeleteA_SPEC_OBJECT_TYPE_REF(c *gin.Context) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("DeleteA_SPEC_OBJECT_TYPE_REF", "GONG__StackPath", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	db.Unscoped().Delete(&a_spec_object_type_refDB)
