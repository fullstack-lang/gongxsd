...
func (backRepoSPEC_RELATION_TYPE *BackRepoSPEC_RELATION_TYPEStruct) CommitPhaseTwoInstance(backRepo *BackRepoStruct, idx uint, spec_relation_type *models.SPEC_RELATION_TYPE) (Error error) {
	...
	if spec_relation_typeDB, ok := backRepoSPEC_RELATION_TYPE.Map_SPEC_RELATION_TYPEDBID_SPEC_RELATION_TYPEDB[idx]; ok {
		...
		// 1. reset
		spec_relation_typeDB.SPEC_RELATION_TYPEPointersEncoding.ALTERNATIVE_ID = make([]int, 0)
		// 2. encode
		for _, a_alternative_idAssocEnd := range spec_relation_type.ALTERNATIVE_ID {
			...
			spec_relation_typeDB.SPEC_RELATION_TYPEPointersEncoding.ALTERNATIVE_ID =
				append(spec_relation_typeDB.SPEC_RELATION_TYPEPointersEncoding.ALTERNATIVE_ID, int(a_alternative_idAssocEnd_DB.ID))
		}

		// 1. reset
		spec_relation_typeDB.SPEC_RELATION_TYPEPointersEncoding.SPEC_ATTRIBUTES = make([]int, 0)
		// 2. encode
		for _, a_spec_attributesAssocEnd := range spec_relation_type.SPEC_ATTRIBUTES {
			...
			spec_relation_typeDB.SPEC_RELATION_TYPEPointersEncoding.SPEC_ATTRIBUTES =
				append(spec_relation_typeDB.SPEC_RELATION_TYPEPointersEncoding.SPEC_ATTRIBUTES, int(a_spec_attributesAssocEnd_DB.ID))
		}
...
func (spec_relation_typeDB *SPEC_RELATION_TYPEDB) DecodePointers(backRepo *BackRepoStruct, spec_relation_type *models.SPEC_RELATION_TYPE) {
	...
	// This loop redeem spec_relation_type.ALTERNATIVE_ID in the stage from the encode in the back repo
	// It parses all A_ALTERNATIVE_IDDB in the back repo and if the reverse pointer encoding matches the back repo ID
	// it appends the stage instance
	// 1. reset the slice
	spec_relation_type.ALTERNATIVE_ID = spec_relation_type.ALTERNATIVE_ID[:0]
	for _, _A_ALTERNATIVE_IDid := range spec_relation_typeDB.SPEC_RELATION_TYPEPointersEncoding.ALTERNATIVE_ID {
		spec_relation_type.ALTERNATIVE_ID = append(spec_relation_type.ALTERNATIVE_ID, backRepo.BackRepoA_ALTERNATIVE_ID.Map_A_ALTERNATIVE_IDDBID_A_ALTERNATIVE_IDPtr[uint(_A_ALTERNATIVE_IDid)])
	}

	// This loop redeem spec_relation_type.SPEC_ATTRIBUTES in the stage from the encode in the back repo
	// It parses all A_SPEC_ATTRIBUTESDB in the back repo and if the reverse pointer encoding matches the back repo ID
	// it appends the stage instance
	// 1. reset the slice
	spec_relation_type.SPEC_ATTRIBUTES = spec_relation_type.SPEC_ATTRIBUTES[:0]
	for _, _A_SPEC_ATTRIBUTESid := range spec_relation_typeDB.SPEC_RELATION_TYPEPointersEncoding.SPEC_ATTRIBUTES {
		spec_relation_type.SPEC_ATTRIBUTES = append(spec_relation_type.SPEC_ATTRIBUTES, backRepo.BackRepoA_SPEC_ATTRIBUTES.Map_A_SPEC_ATTRIBUTESDBID_A_SPEC_ATTRIBUTESPtr[uint(_A_SPEC_ATTRIBUTESid)])
	}
