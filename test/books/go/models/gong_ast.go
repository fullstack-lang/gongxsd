							...
							case *ast.SelectorExpr:
								...
								if Sel := slcExpr.Sel; Sel != nil {
									...
									case "BookDetailsGroup":
										instanceBookDetailsGroup := (&BookDetailsGroup{Name: instanceName}).Stage(stage)
										instance = any(instanceBookDetailsGroup)
										__gong__map_BookDetailsGroup[identifier] = instanceBookDetailsGroup
									case "BookType":
		...
		case *ast.BasicLit, *ast.UnaryExpr:
			...
			case "BookDetailsGroup":
				...
				case "Name":
					...
					__gong__map_BookDetailsGroup[identifier].Name = fielValue
				case "Title":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
					__gong__map_BookDetailsGroup[identifier].Title = fielValue
				case "Author":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
					__gong__map_BookDetailsGroup[identifier].Author = fielValue
				case "Year":
					...
					__gong__map_BookDetailsGroup[identifier].Year = int(exprSign) * int(fielValue)
				case "Format":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
					__gong__map_BookDetailsGroup[identifier].Format = fielValue
				}
			case "BookType":
				switch fieldName {
				// insertion point for field dependant code
