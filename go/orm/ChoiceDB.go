...
func (backRepoChoice *BackRepoChoiceStruct) CommitPhaseTwoInstance(backRepo *BackRepoStruct, idx uint, choice *models.Choice) (Error error) {
	...
	if choiceDB, ok := backRepoChoice.Map_ChoiceDBID_ChoiceDB[idx]; ok {
		...
		// 1. reset
		choiceDB.ChoicePointersEncoding.ModelGroupElements = make([]int, 0)
		// 2. encode
		for _, modelgroupelementAssocEnd := range choice.ModelGroupElements {
			...
			choiceDB.ChoicePointersEncoding.ModelGroupElements =
				append(choiceDB.ChoicePointersEncoding.ModelGroupElements, int(modelgroupelementAssocEnd_DB.ID))
		}
...
func (choiceDB *ChoiceDB) DecodePointers(backRepo *BackRepoStruct, choice *models.Choice) {
	...
	// This loop redeem choice.ModelGroupElements in the stage from the encode in the back repo
	// It parses all ModelGroupElementDB in the back repo and if the reverse pointer encoding matches the back repo ID
	// it appends the stage instance
	// 1. reset the slice
	choice.ModelGroupElements = choice.ModelGroupElements[:0]
	for _, _ModelGroupElementid := range choiceDB.ChoicePointersEncoding.ModelGroupElements {
		choice.ModelGroupElements = append(choice.ModelGroupElements, backRepo.BackRepoModelGroupElement.Map_ModelGroupElementDBID_ModelGroupElementPtr[uint(_ModelGroupElementid)])
	}
