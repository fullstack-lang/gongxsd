...
func (controller *Controller) GetAttribute(c *gin.Context) {
	...
	if err := db.First(&attributeDB, c.Param("id")).Error; err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
		log.Println(err.Error())
...
func (controller *Controller) UpdateAttribute(c *gin.Context) {
	...
	// fetch the attribute
	query := db.First(&attributeDB, c.Param("id"))
	...
	query = db.Model(&attributeDB).Updates(attributeDB)
	if query.Error != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = query.Error.Error()
...
func (controller *Controller) DeleteAttribute(c *gin.Context) {
	...
	db.Unscoped().Delete(&attributeDB)
