...
func (controller *Controller) GetSequence(c *gin.Context) {
	...
	if err := db.First(&sequenceDB, c.Param("id")).Error; err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
		log.Println(err.Error())
...
func (controller *Controller) UpdateSequence(c *gin.Context) {
	...
	// fetch the sequence
	query := db.First(&sequenceDB, c.Param("id"))
	...
	query = db.Model(&sequenceDB).Updates(sequenceDB)
	if query.Error != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = query.Error.Error()
