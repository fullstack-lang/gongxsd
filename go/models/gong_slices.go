...
func EvictInOtherSlices[OwningType PointerToGongstruct, FieldType PointerToGongstruct](
	...
	case *All:
		...
		if fieldName == "ModelGroupElements" {
			...
			for _instance := range *GetGongstructInstancesSetFromPointerType[OwningType](stage) {
				if any(_instance).(*All) != owningInstanceInfered {
					...
					targetFieldSlice := any(_inferedTypeInstance.ModelGroupElements).([]FieldType)
					copy(targetFieldSlice, reference)
					_inferedTypeInstance.ModelGroupElements = _inferedTypeInstance.ModelGroupElements[0:]
					for _, fieldInstance := range reference {
						if _, ok := setOfFieldInstances[any(fieldInstance).(FieldType)]; !ok {
							_inferedTypeInstance.ModelGroupElements =
								append(_inferedTypeInstance.ModelGroupElements, any(fieldInstance).(*ModelGroupElement))
						}
					}
				}
			}
...
func (stage *StageStruct) ComputeReverseMaps() {
	...
	// insertion point per field
	clear(stage.All_ModelGroupElements_reverseMap)
	stage.All_ModelGroupElements_reverseMap = make(map[*ModelGroupElement]*All)
	for all := range stage.Alls {
		_ = all
		for _, _modelgroupelement := range all.ModelGroupElements {
			stage.All_ModelGroupElements_reverseMap[_modelgroupelement] = all
		}
	}
	...
	// insertion point per field
	clear(stage.Choice_ModelGroupElements_reverseMap)
	stage.Choice_ModelGroupElements_reverseMap = make(map[*ModelGroupElement]*Choice)
	for choice := range stage.Choices {
		_ = choice
		for _, _modelgroupelement := range choice.ModelGroupElements {
			stage.Choice_ModelGroupElements_reverseMap[_modelgroupelement] = choice
		}
	}
	...
	// insertion point per field
	clear(stage.ComplexType_ModelGroupElements_reverseMap)
	stage.ComplexType_ModelGroupElements_reverseMap = make(map[*ModelGroupElement]*ComplexType)
	for complextype := range stage.ComplexTypes {
		_ = complextype
		for _, _modelgroupelement := range complextype.ModelGroupElements {
			stage.ComplexType_ModelGroupElements_reverseMap[_modelgroupelement] = complextype
		}
	}
	...
	// insertion point per field
	clear(stage.Extension_ModelGroupElements_reverseMap)
	stage.Extension_ModelGroupElements_reverseMap = make(map[*ModelGroupElement]*Extension)
	for extension := range stage.Extensions {
		_ = extension
		for _, _modelgroupelement := range extension.ModelGroupElements {
			stage.Extension_ModelGroupElements_reverseMap[_modelgroupelement] = extension
		}
	}
	...
	// insertion point per field
	clear(stage.Group_ModelGroupElements_reverseMap)
	stage.Group_ModelGroupElements_reverseMap = make(map[*ModelGroupElement]*Group)
	for group := range stage.Groups {
		_ = group
		for _, _modelgroupelement := range group.ModelGroupElements {
			stage.Group_ModelGroupElements_reverseMap[_modelgroupelement] = group
		}
	}
	...
	// Compute reverse map for named struct ModelGroupElement
	// insertion point per field
	...
	// insertion point per field
	clear(stage.Sequence_ModelGroupElements_reverseMap)
	stage.Sequence_ModelGroupElements_reverseMap = make(map[*ModelGroupElement]*Sequence)
	for sequence := range stage.Sequences {
		_ = sequence
		for _, _modelgroupelement := range sequence.ModelGroupElements {
			stage.Sequence_ModelGroupElements_reverseMap[_modelgroupelement] = sequence
		}
	}
